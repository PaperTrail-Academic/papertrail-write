...
function renderDetailRow(sub) {
  ...
  const resubmitBtn = sub.is_submitted
    ? `<button class="btn btn-ghost btn-sm" style="font-size:var(--text-xs);color:var(--pt-muted);border-color:var(--pt-border)" onclick="event.stopPropagation();unsubmitStudent('${sub.id}','${esc(sub.student_display_name)}')" title="Let this student re-enter the writing environment">↩ Allow Resubmit</button>`
    : '';
  return `<tr class="detail-row"><td class="detail-cell" colspan="7"><div class="detail-header"><div><strong>${esc(sub.student_display_name)}</strong><span style="color:var(--pt-muted);font-size:var(--text-xs);margin-left:0.5rem">${sub.word_count||0} words · Started ${formatTime(sub.started_at)}</span></div><div style="display:flex;align-items:center;gap:var(--space-md)"><div style="font-size:var(--text-xs);color:var(--pt-muted)">${flagText||'No notable events'}</div>${resubmitBtn}</div></div><div class="detail-essay">${esc(sub.essay_text||'(no essay text)')}</div><div class="process-log-title">Process Log</div><div class="process-log-list">${logHtml}</div><div style="margin-top:var(--space-sm)"><div class="disclaimer">This log is one input among many. Educator judgment governs all interpretation and any subsequent conversation.</div></div></td></tr>`;
}
